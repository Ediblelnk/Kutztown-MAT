...
\begin{enumerate}
    ...
    \qitem{46}{Let $a \tand b$ be elements of a group $G$. Show that if $ab$ has finite order $n$, then $ba$ also has order $n$}{
        ...
        \begin{proof}
            ...
            \begin{align*}
                (ab)^n & = e && \text{for some smallest positive integer $n$}\\
                a(ba)^{n-1}b & = e \\
                ba(ba)^{-1}ba & = ba \\
                (ba)^{n+1} & = ba \\
                (ba)^n & = e
            \end{align*}
            Thus, $ba$ has order $\leq n$. Assume that $ba$ has order $< n$, say $m$. Thus, $(ba)^m = e$. By a similar argument as above, we can show that $(ab)^m = e$. This contradicts our assumption that the order of $ab$ is $n$.

            $\therefore$ the order of $ba$ must be exactly $n$.
        \end{proof}
    }
\end{enumerate}
