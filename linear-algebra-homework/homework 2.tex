...
\begin{enumerate}
  ...
  \begin{proof}
    ...
    \end{align*}
    $\therefore \vec{v} = (0,0,0)$ is the additive identity for $V$, \bld{id}.
  \end{proof}
  \bitem{Axiom 5}
  \begin{proof}
...
\begin{enumerate}
  \bitem{a} \bld{A vector is a directed line segment (an arrow)}
  \item[] False, A vector can be anything you can imagine, from a function to fruit.
    \bitem{b} \bld{A vector is an \itl{n}-tuple of real numbers.}
  \item[] False, A vector can be anything you can imagine, including an \itl{n}-tuple of real numbers, but it doesn't have to be.
    \bitem{c} \bld{A vector is any element of a vector space}
  \item[] True, A vector \bld{is} any element of a vector space
    \bitem{e} \bld{The set of polynomials with degree \itl{exactly} 1 is a vector space under the operation defined in Example 7.}
